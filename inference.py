...
class Inference(object):
    def __init__(self, config) -> None:
        ...
        # self.model = load_model(args.load_ckpt)
    ...
    def inference(self, text):
        input_ids, attention_mask, first_subword, char_ids, seq_len = self.preprocessing(
            text)
        ...
        for i in range(len(input_tensor)):
            for j in range(i, len(input_tensor)):
                if input_tensor[i][j] > 0:
                    tmp = [i, j, self.label_set[int(input_tensor[i][j])], float(
                        pred[i][j][int(input_tensor[i][j])])]
                    entity.append(tmp)
        return entity
    ...
    def revised_output(self, result, text):
        ...
        if not result:
            ...
            for span in result:
                ...
                tmp = re.search(
                    '^((sỹ)|(Sỹ)|(SỸ)|(sỸ)|(sy)|(Sy)|(sY)|(SY))\s+', value)
                if tmp:
                    tmp_2 = re.search(
                        '((bác)|(Bác)|(bÁc)|(báC)|(BÁc)|(BáC)|(bÁC)|(BÁC)|(bac)|(Bac)|(bAc)|(baC)|(BAc)|(BaC)|(bAC)|(BAC))$', text[:start])
                    if tmp_2:
                        value = value[tmp.span()[1]:]
                        start = start + tmp.span()[1]
                revised.append({
